...
export default function ScanScreen({ onScan, onHistory }) {
  ...
  const runMobileNetLoop = async () => {
    ...
    try {
      // 1. Ask custom model what it sees (only inside detected fruit box)
      const bbox = bestFruitBoxRef.current;
      if (!bbox) {
        ...
        mnetLoopRef.current = setTimeout(runMobileNetLoop, 450);
        return;
      }

      const [bx, by, bw, bh] = bbox;
      const roi = roiCanvasRef.current || document.createElement("canvas");
      roiCanvasRef.current = roi;
      const rx = Math.max(0, Math.floor(bx));
      const ry = Math.max(0, Math.floor(by));
      const rw = Math.max(1, Math.min(video.videoWidth - rx, Math.floor(bw)));
      const rh = Math.max(1, Math.min(video.videoHeight - ry, Math.floor(bh)));
      roi.width = rw;
      roi.height = rh;
      roi.getContext("2d").drawImage(video, rx, ry, rw, rh, 0, 0, rw, rh);
      const preds = await mnet.predict(roi);

      let match = null;
      for (const p of preds) {
        if (p.probability > 0.60 && p.className !== "Background") {
          ...
          if (classNameStr.includes("dalandan") || classNameStr.includes("orange")) mappedKey = "orange";

          const label = p.className.toLowerCase();
          const parsedCondition = label.includes("rotten") || label.includes("bulok")
            ? "rotten"
            : label.includes("unripe") || label.includes("hilaw")
            ? "unripe"
            : label.includes("overripe") || label.includes("sobrang")
            ? "overripe"
            : label.includes("fresh") || label.includes("ripe") || label.includes("sariwa") || label.includes("hinog")
            ? "ripe"
            : null;

          const parsedFruit = p.className
            .replace(/rotten|overripe|unripe|ripe|fresh|bulok|sariwa|hinog|hilaw/gi, "")
            .replace(/[_-]+/g, " ")
            .replace(/\s+/g, " ")
            .trim();

          match = { 
            fruit: parsedFruit || p.className,
      ...
      // 2. THIS IS THE TIMER I ACCIDENTALLY DELETED!
      if (match) {
        setDetected(match); // Updates the text pill UI
        ...
        window.__siglaani_class_condition__ = match.parsedCondition;

        // Start the 3-second countdown interval
        if (!detectStart.current) {
          ...
          timerRef.current = setInterval(() => {
            ...
            if (secs <= 0) {
              clearInterval(timerRef.current);
              captureAndProceed(); // 📸 This actually snaps the picture!
            }
          }, 1000);
        }
      } else {
        // Reset timer if the fruit is moved away before 3 seconds
        if (detectStart.current) {
          detectStart.current = null;
          clearInterval(timerRef.current);
          setCountdown(3);
 ...
 const captureAndProceed = () => {
    ...
    if (video && cap) {
         // Crop to detected fruit box first so analysis ignores most of the background.
      const [bx, by, bw, bh] = bestFruitBoxRef.current ?? [0, 0, video.videoWidth, video.videoHeight];
      const padX = bw * 0.12;
      const padY = bh * 0.12;
      const sx = Math.max(0, Math.floor(bx - padX));
      const sy = Math.max(0, Math.floor(by - padY));
      const sw = Math.min(video.videoWidth - sx, Math.floor(bw + padX * 2));
      const sh = Math.min(video.videoHeight - sy, Math.floor(bh + padY * 2));
      ...
      cap.width  = targetW;
      cap.height = targetH;
      cap.getContext("2d").drawImage(video, sx, sy, sw, sh, 0, 0, targetW, targetH);

      // Save it at 80% quality to ensure a lightning-fast upload
      window.__siglaani_capture__ = cap.toDataURL("image/jpeg", 0.80);
    }
